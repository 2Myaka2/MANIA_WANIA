...
| Window-level RIN | Temporal RIN | Analysis v1.3 reference; Stage 22.A boundary | `src/mania/analysis/temporal.py` — defines input and window metadata only; it does not construct graphs or calculate window frequency. | missing | MVP candidate | MANIA scientific MVP | Stage 22.A does not infer absent numeric frames, apply `TEMP_MIN_FREQ`, export `temporal_rin_{condition}.csv`, or change WANIA rendering. | Stage 22.B only after explicit approval |
