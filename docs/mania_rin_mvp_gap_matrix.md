...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `residue_table_{cond}.csv` | Preprocessing artifact | Stage 19 context; preprocessing v1.2 reference | `src/mania/adapters/notebook_export.py` — consumes this exact notebook filename and maps it to backend `nodes.csv`; `data/reference/notebooks_libraries_v1_1/results/preprocess_export_v1.1/residue_table_normal.csv` — provides a reference example. | partially covered | MVP candidate | MANIA scientific MVP | Production workflow writes `graph/nodes.csv`, not this name. Contract naming/layout mismatch. | Stage 19.2 |
| `protein_contact_edges_undirected_{cond}.csv` | Preprocessing artifact | Stage 19 context; preprocessing v1.2 reference | `src/mania/adapters/notebook_export.py` — consumes the exact notebook filename and maps it to backend edges; `src/mania/preprocessing/trajectory_graph_workflow.py` — writes production `graph/edges.csv`. | partially covered | MVP candidate | MANIA scientific MVP | Adapter coverage exists, but the production artifact name/layout differs. | Stage 19.2 |
| `contacts_per_frame_{cond}.parquet` | Preprocessing artifact | Stage 19 context | `docs/notebook_gap_report_v1_3.md` — records implemented per-frame semantics at `contacts/contacts_perframe.csv` and explicitly defers parquet parity. | partially covered | MVP candidate | MANIA scientific MVP | Contract naming mismatch: task `contacts_per_frame`, notebook/reference `contacts_perframe`, backend `contacts/contacts_perframe.csv`; format mismatch: parquet versus CSV. | Stage 19.2 |
| `non_protein_nodes_{cond}.csv` | Preprocessing artifact | Stage 19 context | `data/reference/notebooks_libraries_v1_3/MANIA_preprocessing_v1_2.ipynb` — writes `nonprotein_nodes_{condition}.csv`; historical examples exist under `data/reference/notebooks_libraries_v1_1/results/preprocess_export_v1.1/`. | partially covered | optional candidate | optional scientific artifact | Reference-only evidence; no exact production writer. Naming gap: `non_protein_nodes` versus `nonprotein_nodes`. Must not become required WANIA render data. | Stage 19.2 |
| `np_contact_edges_{cond}.csv` | Preprocessing artifact | Stage 19 context; preprocessing v1.2 reference | `data/reference/notebooks_libraries_v1_3/MANIA_preprocessing_v1_2.ipynb` — defines the export; `data/reference/notebooks_libraries_v1_1/results/preprocess_export_v1.1/np_contact_edges_normal.csv` — shows its historical schema. | partially covered | optional candidate | optional scientific artifact | Reference artifact exists, but no production writer/contract was found. | Stage 19.2 |
...
| `edge_semantics.json` | Preprocessing artifact | Stage 19 context; preprocessing v1.2 reference | `data/reference/notebooks_libraries_v1_1/results/preprocess_export_v1.1/edge_semantics.json` — concrete reference artifact; `src/mania/constants.py` — freezes only the current backend priority vocabulary. | partially covered | MVP candidate | MANIA scientific MVP | No current production writer was found; the constant does not encode the full notebook criteria. | Stage 19.2 |
| `mania_manifest.json` | Preprocessing artifact | Stage 19 context; preprocessing v1.2 reference | `src/mania/adapters/notebook_export.py` — validates and consumes `mania_manifest.json`; `tests/test_notebook_export_adapter.py` — tests that adapter path. | partially covered | backend/internal | backend-only/internal | Consumed as a notebook manifest, while current backend workflow metadata/layout differs; canonical output role needs confirmation. | Stage 19.2 |
| `mania_residue_library.json` | Preprocessing artifact | Stage 19 context; preprocessing v1.2 reference | `src/mania/preprocessing/residue_library_validation.py` — validates residue-library data; `src/mania/preprocessing/residue_library_bridge.py` — bridges it into preprocessing. | partially covered | backend/internal | backend-only/internal | The library is an input/reference source, not a proven per-run emitted artifact. Final biological residue lists remain outside this stage. | Stage 19.2 |
...
| `hbond`, `disulfide`, `vdw`, `hydrophobic`, `ionic`, `salt_bridge` | Edge semantics | Preprocessing v1.2 reference | `src/mania/constants.py` — recognizes these names in `EDGE_TYPE_PRIORITY`; `data/reference/notebooks_libraries_v1_3/MANIA_preprocessing_v1_2.ipynb` — contains the reference detection functions/criteria. | partially covered | MVP candidate | MANIA scientific MVP | Recognized contract vocabulary and notebook reference logic exist, but current production contact chemistry does not compute these types. | Stage 19.2 |
...
- `contacts_per_frame_{cond}.parquet` (Stage 19 list) versus
  ...
  versus `contacts/contacts_perframe.csv` (current backend): naming, layout,
  and format gaps.
