...
| `mania analyze` | Analysis orchestration CLI | Stage 24.C roadmap | `src/mania/analysis/orchestration.py` — validates requested Stage 20 artifact roots and conditions, orchestrates accepted Stage 21/22 APIs, writes the accepted `analysis/` layout, preserves disabled-PCA and fingerprint-clustering defaults, and returns deterministic stdout JSON; `tests/test_cli_analyze.py` covers help, default/PCA/PCA-clustering runs, invalid combinations, input validation, single/multi-condition comparison, determinism, no raw-MD calls, and WANIA non-invocation. | covered at Stage 24.C | MVP candidate | MANIA scientific MVP | This is a backend orchestration command only. It does not read raw trajectories, rerun preprocessing, invoke WANIA, change WANIA required fields/runtime schema/adapter/capabilities/artifact mapping, or add API/frontend/Docker/database/worker code. The stdout JSON remains a compact command summary, not the persisted manifest. | Stage 24.C complete |
| `extended_metrics.json` | Analysis manifest | Stage 24.D roadmap | `src/mania/analysis/extended_metrics.py` — writes `analysis/extended_metrics.json` with schema version `mania.extended_metrics.v0.1` from current-run `AnalyzeRunResult` artifact ownership; `tests/test_analysis_extended_metrics.py` covers default, PCA, PCA-clustering, degenerate PCA, single-condition comparison, relative-path containment, stale-file exclusion, deterministic bytes, no embedded scientific tables, and WANIA non-integration. | covered at Stage 24.D | backend/internal | MANIA scientific MVP | MANIA-only compact index/status manifest. It records condition-level and run-level statuses, PCA intent versus computation status, clustering provenance, requested/used PCA components, diagnostics, limitations, and portable `analysis/` artifact references. It does not duplicate scientific rows, parse stdout, scan stale output files, alter WANIA payloads, or implement API/frontend integration. | Stage 24.D complete; Stage 24.E final validation/docs/acceptance remains separate |
