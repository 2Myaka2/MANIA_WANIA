...
| Requirement | Requirement group | Source / context | Repo evidence | Current repo status | Classification | Layer | Gap / notes | Suggested follow-up stage |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `graph.json` | Analysis artifact | Backend contract; Stage 21.A | `src/mania/analysis/static_rin_graph.py` — deterministically builds and writes a per-condition static analysis graph from accepted Stage 20 residue/contact artifacts; focused synthetic tests cover identity, priority, overlap, metrics preservation, missing values, ordering, and portable provenance. The older `src/mania/preprocessing/trajectory_graph_export.py` workflow graph remains separate. | covered at Stage 21.A analysis baseline | accepted required | MANIA scientific MVP | Recommended analysis layout is `analysis/{condition}/graph.json`. It reuses central `EDGE_TYPE_PRIORITY`, retains all typed aggregates, and is neither the older workflow `graph/graph.json` nor the WANIA frontend payload. | Stage 21.A complete; Stage 21.B requires separate approval |
...
| `communities_{cond}.csv` | Analysis artifact | Analysis v1.3 reference; Stage 21.C | `src/mania/analysis/static_rin_communities.py` — consumes the accepted Stage 21.A graph and writes deterministic `communities_<condition>.csv`; focused synthetic tests cover identity, disconnected graphs, isolates, missing weights, quality metrics, and stable export. The older `graph_metrics_export.py` path remains separate. | covered for the Stage 21.A graph by Stage 21.C | accepted required | MANIA scientific MVP | Includes Stage 21.A node identity, community size, truthful algorithm name, modularity, and community count; not required in WANIA payload rows. | Stage 21.C complete; consumed by Stage 21.D |
| `region_enrichment_{cond}.csv` | Analysis artifact | Stage 21.D | `src/mania/analysis/static_rin_region_enrichment.py` — joins accepted Stage 21.A node labels to Stage 21.C assignments and writes deterministic computed or skipped rows. | covered when existing region labels provide valid margins; otherwise explicitly skipped | optional candidate | Optional scientific artifact | Uses labeled nodes only, reports raw two-sided Fisher p-values with method/status metadata, and never infers regions from residue numbers. | Stage 21.D complete; Stage 21.E requires separate approval |
| `stats.csv`, `comparison.csv` | Analysis artifact | Analysis v1.3 reference; backend contract skeleton; Stage 21.E | `src/mania/analysis/static_rin_comparison.py` — consumes accepted Stage 21.B centrality artifacts, matches stable residue identity, and writes deterministic root-level comparison and method/status rows. | covered for conservative node-metric comparisons | MVP candidate | MANIA scientific MVP | Missing values remain missing; identity conflicts and deferred edge/community/region scopes are explicit. The Stage 21.E scientific schema does not modify the planned WANIA-facing skeleton. | Stage 21.E complete; Stage 21.F requires separate approval |
...
| `community` | Analysis metrics | Analysis v1.3 reference; Stage 21.C | `src/mania/analysis/static_rin_communities.py` — assigns stable one-based communities on the accepted Stage 21.A topology and explicitly reports `greedy_modularity_unweighted`. | covered for the Stage 21.A graph by Stage 21.C | accepted required | MANIA scientific MVP | Louvain is unavailable within the accepted dependency boundary; the deterministic fallback is not mislabeled. | Stage 21.C complete; Stage 21.D requires separate approval |
| `modularity` | Analysis metrics | Analysis v1.3 reference; Stage 21.C | `src/mania/analysis/static_rin_communities.py` — computes deterministic unweighted modularity and repeats it in each condition artifact row. | covered for the Stage 21.A graph by Stage 21.C | accepted required | MANIA scientific MVP | Community quality is backend scientific output, not a required WANIA field. | Stage 21.C complete; Stage 21.D requires separate approval |
...
| `Fisher enrichment` | Analysis metrics | Analysis v1.3 reference; Stage 21.D | `src/mania/analysis/static_rin_region_enrichment.py` — computes dependency-free two-sided Fisher exact tests for valid community/region 2x2 tables and emits explicit skipped statuses otherwise. | covered for existing Stage 21.A region labels by Stage 21.D | optional candidate | optional scientific artifact | Missing labels are excluded, not classified; no region-number mapping, correction, or cross-condition claim is introduced. | Stage 21.D complete; Stage 21.E requires separate approval |
