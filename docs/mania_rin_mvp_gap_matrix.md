...
| `conformation_pca_{cond}.csv` | Analysis artifact | Analysis v1.3 reference; Stage 22.E/22.G | `src/mania/analysis/conformation_pca.py` — consumes accepted Stage 22.D fingerprints and atomically writes a deterministic per-frame artifact; Stage 22.G validates schema, bytes, frame identity, and blank unavailable fields. | covered and validated with explicit unavailable/skipped statuses | optional candidate | optional scientific artifact | The accepted dependency boundary has no direct numerical linear-algebra backend. Component coordinates and explained-variance ratios remain blank, with `n_components = 0`; no fake PCA is emitted. | Stage 22.G complete; computed PCA requires explicit future numerical-backend approval |
...
| `k-means`, `silhouette`, `conformation_labels_{condition}.csv` | Conformation | Analysis v1.3 reference; Stage 22.F/22.G | `src/mania/analysis/conformation_clustering.py` — consumes the Stage 22.D binary contact fingerprint matrix, runs deterministic dependency-free k-means, scores valid candidate label sets, stabilizes state IDs, and atomically writes per-frame labels; Stage 22.G contract tests validate schema, bytes, frame coverage, metadata, and skipped values. | covered from contact fingerprints and validated at Stage 22.G | MVP candidate | MANIA scientific MVP | PCA coordinates remain unavailable and are not clustering input. The artifact records fingerprint input and does not claim notebook PCA-to-k-means parity. Full parity requires future explicit numerical-backend approval. | Stage 22.G complete |
| Representative frames | Conformation | Analysis v1.3 reference; Stage 22.F/22.G | `src/mania/analysis/conformation_clustering.py` — selects the frame nearest each computed fingerprint centroid and breaks ties by `frame_index`; Stage 22.G tests require exactly one representative per computed state and none for skipped rows. | covered and validated for computed clusters | optional candidate | optional scientific artifact | Do not confuse conformation representatives with representative first-frame Cα coordinates. | Stage 22.G complete |
