...
| `temporal_rin_{cond}.csv` | Analysis artifact | Analysis v1.3 reference; Stage 22.C | `src/mania/analysis/temporal_rin_metrics.py` — computes deterministic metrics from accepted Stage 22.B window graphs and atomically writes one row per window. | covered at Stage 22.C | MVP candidate | MANIA scientific MVP | Stable identity/status fields accompany counts, density, degree/strength, unweighted path summaries, and truthful `greedy_modularity_unweighted` community summaries. The artifact remains separate from WANIA base rendering. | Stage 22.C complete |
...
| `conformation_pca_{cond}.csv` | Analysis artifact | Analysis v1.3 reference; Stage 22.E | `src/mania/analysis/conformation_pca.py` — consumes accepted Stage 22.D fingerprints and atomically writes a deterministic per-frame artifact. | covered with explicit unavailable/skipped statuses at Stage 22.E | optional candidate | optional scientific artifact | The accepted dependency boundary has no direct numerical linear-algebra backend. Component coordinates and explained-variance ratios remain blank, with `n_components = 0`; no fake PCA is emitted. | Stage 22.E complete; computed PCA requires an explicit dependency decision |
...
| `PCA` | Conformation | Analysis v1.3 reference; Stage 22.E | `src/mania/analysis/conformation_pca.py` — validates and deterministically centers binary Stage 22.D values to identify zero total variance, preserves frame metadata, and exports the stable artifact contract. | unavailable within the accepted numerical dependency boundary | MVP candidate | MANIA scientific MVP | Nonconstant inputs use `pca_unavailable`; degenerate inputs use explicit statuses. No coordinates, explained variance, component signs, or component counts are invented. | Stage 22.E complete; computed centered SVD requires explicit dependency approval |
| `k-means`, `silhouette`, `conformation_labels_{condition}.csv` | Conformation | Analysis v1.3 reference; Stage 22.F | `src/mania/analysis/conformation_clustering.py` — consumes the Stage 22.D binary contact fingerprint matrix, runs deterministic dependency-free k-means, scores valid candidate label sets, stabilizes state IDs, and atomically writes per-frame labels. | covered from contact fingerprints at Stage 22.F | MVP candidate | MANIA scientific MVP | PCA coordinates remain unavailable and are not clustering input. The artifact records fingerprint input and does not claim notebook PCA-to-k-means parity. Full parity requires future explicit numerical-backend approval. | Stage 22.F complete; Stage 22.G remains separate |
| Representative frames | Conformation | Analysis v1.3 reference; Stage 22.F | `src/mania/analysis/conformation_clustering.py` — selects the frame nearest each computed fingerprint centroid and breaks ties by `frame_index`. | covered for computed clusters at Stage 22.F | optional candidate | optional scientific artifact | Skipped inputs receive no representative. Do not confuse these frames with representative first-frame Cα coordinates. | Stage 22.F complete; Stage 22.G remains separate |
