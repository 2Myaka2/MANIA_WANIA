...
| `temporal_rin_{cond}.csv` | Analysis artifact | Analysis v1.3 reference; Stage 22.B boundary | `src/mania/analysis/temporal_rin_windows.py` — constructs deterministic in-memory window RINs but deliberately has no artifact writer. | missing as a public artifact | MVP candidate | MANIA scientific MVP | Stage 22.B graph construction does not establish temporal metrics or CSV export coverage. The future artifact remains optional for WANIA base rendering. | Stage 22.C only after explicit approval |
