...
| `mania analyze` | Analysis orchestration CLI | Stage 24.C roadmap; Stage 24.F correction | `src/mania/analysis/orchestration.py` — validates requested Stage 20 artifact roots and conditions, orchestrates accepted Stage 21/22 APIs, writes the accepted `analysis/` layout only after fatal requested PCA checks, preserves disabled-PCA and fingerprint-clustering defaults, and returns deterministic stdout JSON; `tests/test_cli_analyze.py` covers help, default/PCA/PCA-clustering runs, invalid combinations, input validation, fatal requested PCA failure before writes, single/multi-condition comparison, determinism, no raw-MD calls, and WANIA non-invocation. | covered and validated through Stage 24.F | MVP candidate | MANIA scientific MVP | This is a backend orchestration command only. It does not read raw trajectories, rerun preprocessing, invoke WANIA, change WANIA required fields/runtime schema/adapter/capabilities/artifact mapping, or add API/frontend/Docker/database/worker code. The stdout JSON remains a compact command summary, not the persisted manifest. | Stage 24.F complete; Stage 24 closed |
