...
is maintained through the Stage 24.G preprocessing-to-analysis CLI integration. It maps the
...
| `conformation_labels_{cond}.csv` | Analysis artifact | Analysis v1.3 reference; Stage 22.F/22.G; Stage 24.F | `src/mania/analysis/conformation_clustering.py` — writes deterministic fingerprint-based labels and representatives, supports explicit PCA-basis labels, and preserves PCA projection status as provenance when fingerprint clustering remains active; Stage 24.F tests cover computed/skipped PCA provenance, PCA-used state, component selection above three, and no silent fallback. | covered and validated as `conformation_labels_{condition}.csv` | MVP candidate | MANIA scientific MVP | The accepted artifact deliberately does not claim notebook PCA-to-k-means parity and remains separate from planned legacy `conformational_states.csv` naming. Fingerprint clustering may truthfully report computed-but-unused PCA. | Stage 24.F complete; Stage 24 closed |
| `conformation_pca_{cond}.csv` | Analysis artifact | Analysis v1.3 reference; Stage 22.E/22.G; Stage 24.F | `src/mania/analysis/conformation_pca.py` — consumes accepted Stage 22.D fingerprints, atomically writes the stable PC1-PC3 per-frame artifact, and retains up to 10 internal finite score components for PCA clustering when `enable_pca=True`; Stage 24.F validates component bounds, deterministic full scores, CSV field order, and expected numerical failure status. | covered and validated with explicit disabled/skipped/computed/failed statuses | optional candidate | optional scientific artifact | Default PCA remains disabled and preserves blank `pca_unavailable` rows with `n_components = 0`. Explicit PCA uses only `ContactFingerprintMatrix.values`, emits finite rank-supported internal coordinates and explained-variance ratios, leaves unavailable exported components blank, and does not add PC4+ CSV columns. | Stage 24.F complete; Stage 24 closed |
| `mania analyze` | Analysis orchestration CLI | Stage 24.C roadmap; Stage 24.G compatibility | `src/mania/analysis/orchestration.py` — validates requested Stage 20 artifact roots and conditions, uses portable `mania_manifest.json` artifact references when present so filename-normalized Stage 20 conditions are accepted, orchestrates accepted Stage 21/22 APIs, writes the accepted `analysis/` layout only after fatal requested PCA checks, preserves disabled-PCA and fingerprint-clustering defaults, and returns deterministic stdout JSON; `tests/test_cli_analyze.py` and `tests/test_cli_preprocessing_analysis_inputs.py` cover help, default/PCA/PCA-clustering runs, invalid combinations, input validation, fatal requested PCA failure before writes, single/multi-condition comparison, safe preprocessing-to-analysis roots, determinism, no raw-MD calls, and WANIA non-invocation. | covered and validated through Stage 24.G | MVP candidate | MANIA scientific MVP | This is a backend orchestration command only. It does not read raw trajectories, rerun preprocessing, invoke WANIA, change WANIA required fields/runtime schema/adapter/capabilities/artifact mapping, or add API/frontend/Docker/database/worker code. The stdout JSON remains a compact command summary, not the persisted manifest. | Stage 24.G compatibility complete |
| `mania preprocessing run-graph-export --export-analysis-inputs` | Preprocessing-to-analysis CLI integration | Stage 24.G | `src/mania/cli.py` and `src/mania/preprocessing/trajectory_graph_workflow.py` — add an explicit disabled-by-default flag that reuses the accepted Stage 20 residue table, protein contact, and manifest writers after graph export succeeds; `tests/test_cli_preprocessing_analysis_inputs.py` validates one-condition, two-condition, safe filename, writer-failure, legacy-coexistence, and direct `mania analyze` compatibility. | covered and validated at Stage 24.G | accepted required | MANIA scientific MVP | Writes root-level analysis-ready Stage 20 inputs directly under `--output`; requires `--contact-selection protein`; rejects `--skip-contacts`; does not run analysis; does not change legacy `--export-scientific-csvs`; does not invoke WANIA. | Stage 24.G complete |
| `extended_metrics.json` | Analysis manifest | Stage 24.D roadmap; Stage 24.F correction | `src/mania/analysis/extended_metrics.py` — writes `analysis/extended_metrics.json` with schema version `mania.extended_metrics.v0.1` from current-run `AnalyzeRunResult` artifact ownership; `tests/test_analysis_extended_metrics.py` covers default, PCA, PCA-clustering, degenerate PCA, single-condition comparison, truthful PCA status, explicit PCA-used-for-clustering state, component counts, relative-path containment, stale-file exclusion, deterministic bytes, no embedded scientific tables, and WANIA non-integration. | covered and validated through Stage 24.F | backend/internal | MANIA scientific MVP | MANIA-only compact index/status manifest. It records condition-level and run-level statuses, PCA intent versus computation status, maximum/computed/exported PCA component counts, clustering basis/provenance, explicit PCA use, requested/used PCA components, diagnostics, limitations, and portable `analysis/` artifact references. It does not duplicate scientific rows, parse stdout, scan stale output files, alter WANIA payloads, or implement API/frontend integration. | Stage 24.F complete; Stage 24 closed |
...
| `PCA` | Conformation | Analysis v1.3 reference; Stage 22.E; Stage 24.F | `src/mania/analysis/conformation_pca.py` — validates binary Stage 22.D values, preserves frame metadata, exports the stable PC1-PC3 artifact contract, computes optional centered NumPy SVD PCA only when `enable_pca=True`, and retains up to 10 internal components for PCA clustering. | optional computed PCA covered | MVP candidate | MANIA scientific MVP | Default nonconstant inputs still use `pca_unavailable` with blank coordinates. Explicit PCA computes rank-supported internal components with deterministic sign stabilization and finite explained-variance ratios; public CSV export remains PC1-PC3 only. Degenerate inputs remain nonfatal skipped outcomes, while requested numerical PCA failure is fatal in `mania analyze`. | Stage 24.F complete |
| `k-means`, `silhouette`, `conformation_labels_{condition}.csv` | Conformation | Analysis v1.3 reference; Stage 22.F/22.G; Stage 24.F | `src/mania/analysis/conformation_clustering.py` — consumes the Stage 22.D binary contact fingerprint matrix by default, or a computed in-memory PCA projection when `clustering_basis="pca"` is explicit. Both bases reuse deterministic dependency-free k-means, valid silhouette scoring, state-ID stabilization, representative selection, and atomic per-frame label export; Stage 24.F tests validate schema, bytes, frame coverage, truthful PCA provenance, skipped values, PCA component selection above three, and identity checks. | covered from contact fingerprints and optional PCA coordinates | MVP candidate | MANIA scientific MVP | Fingerprint clustering records binary input and preserves computed/skipped/unavailable PCA status as provenance without using PCA. Optional PCA mode records `deterministic_kmeans_pca`, `conformation_pca_coordinates`, `pca_status = computed`, and uses all internal computed components by default. PCA is not silently enabled, supplying a projection alone does not switch modes, invalid PCA configuration does not fall back to fingerprints, and notebook PCA-to-k-means parity is not claimed. | Stage 24.F complete |
