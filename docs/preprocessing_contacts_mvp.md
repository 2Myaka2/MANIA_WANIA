...
`resid_j`, preserving condition scope in `condition`, mapping the
priority-selected `edge_kind` to `edge_type`, writing all unique edge types to
`all_edge_types`, writing their count to `n_edge_types`, mapping
`contact_frequency` to `contact_freq`, and mapping angstrom-labelled
`mean_minimum_distance` to `mean_dist_A`.

Stage 14.1c-fix treats
`data/reference/notebooks_libraries_v1_2/MANIA_analysis_v1_2.ipynb` as the
current graph reference semantics. v1.2 Cell 5 adds interaction priority for
graph edge display. v1.2 Cell 12 fixes temporal RIN export handling, but that
temporal RIN fix is documented-only here and remains future temporal/workflow
artifact scope. Current Stage 13 contacts still emit one generic
`residue_contact` edge type: `edge_type = residue_contact`,
...
## Stage 14.1f transition

Stage 13 Contacts MVP remains complete. Stage 14.1f adds a graph export
bundle boundary for existing Stage 14 graph artifacts. The bundle consumes
existing nodes.csv, corrected edges.csv, and graph.json artifacts, uses
Stage 14.1d CSV validation for consistency, and checks lightweight graph JSON
structure/count consistency.

Stage 13 contact_edges.csv is aggregate contacts table output. backend graph
edges.csv and graph.json are Stage 14 graph artifacts. Stage 14 graph export
bundle metadata is also a Stage 14 graph boundary and does not make Stage 13
contact_edges.csv the backend graph edges.csv.
...
the accepted display priority, v1.2 Cell 12 temporal RIN export fix is
documented, and temporal RIN export remains future scope.

Stage 14.1f does not run graph validators or diagnostics, graph comparison,
...
## Stage 14.3b transition

Stage 14.3b compares generated graph artifacts with notebook reference artifacts v1.2 and uses Stage 14.3a input contract. It compares enabled generated/reference `nodes.csv`, corrected backend graph `edges.csv`, and `graph.json` artifacts only after Stage 14.3a validation passes.

The comparison is read-only and dependency-free. It reports exact generated/reference differences without explaining them; expected mismatch documentation remains Stage 14.4a.
