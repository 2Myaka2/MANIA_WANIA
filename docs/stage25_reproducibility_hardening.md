...
E.2 integrates completed preprocessing `runtime_metadata.json` and `pbc_audit.json`,
and completed analysis `analysis/runtime_metadata.json`. PBC observes selected
frames only during the existing Rg pass, or the contacts pass when Rg is disabled;
there is no extra metadata trajectory pass or double-count when both run.
Runtime reuses Stage 25.B timestamps and retained counters without another clock
read or recomputation. Successfully written artifacts enter the existing inventory
none/sha256 mode and portable provenance references, followed by strict unified
validation. Failed scientific runs retain the existing Stage 25.B/C boundary and
do not automatically persist E.2 artifacts. No scientific output or CLI count changes.

Automatic external PBC preprocessing remains undeclared; scientific PBC status
