"""Opt-in residue-name extraction check for local loaded runtimes."""
...
def test_extract_local_trajectory_residue_names() -> None:
    ...
    report = extract_manifest_residue_names(load_result)

    assert isinstance(report, PreprocessingManifestResidueNames)
    assert report.condition_residue_names
